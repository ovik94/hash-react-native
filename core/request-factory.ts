...
export default class RequestFactory {
  ...
  protected onError = <T>(response: AxiosResponse): Promise<IResponseData<T>> => {
  ...
  public createRequest<T = any>(
    ...
    const { path, method, headers: methodHeaders } = this.options.requestConfigList[requestId];
    ...
    return axios.request<T, AxiosResponse<IResponseData<T>>>(requestConfig)
      .then(response => this.onSuccess<T>(response))
      .catch(err => this.onError<T>(err.response));
  }
}
